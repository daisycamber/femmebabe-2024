...
def buy_photo_card(request, username):
    ...
    if not request.GET.get('id'): return redirect(request.path + get_qs(request.GET) + '&id={}'.format(Post.objects.filter(author=user, private=False, public=False, published=True, recipient=None).exclude(image=None).order_by('?').first().uuid))
    id = request.GET.get('id', None)
    post = Post.objects.get(uuid=id)
