...
def feed_context(request):
    ...
    if hasattr(request, 'user'):
        user = request.user
        context_data['is_admin'] = request.user == User.objects.get(id=2) if User.objects.count() > 1 else False or request.user.is_superuser
    else:
...
#    except: pass
    ...
    if hasattr(request, 'user') and request.user.is_authenticated and ip != None and ip.latitude != None and ip.longitude != None:
        ...
        context_data['clock_color'] = '#ffcccb' if h >= 9 and h < 21 else 'lightblue'
    context_data['securitymodal'] = (not request.path.startswith('/verify/age/') and not request.path.startswith('/accounts/tfa/') and not request.path.startswith('/security/mrz/') and not request.path.startswith('/security/nfc/')) and (hasattr(request, 'user') and request.user.is_authenticated and request.user.profile.vendor and not face_mrz_or_nfc_verified(request))
    context_data['securitymodaljs'] = (not request.path.startswith('/verify/age/') and not request.path.startswith('/accounts/tfa/') and not request.path.startswith('/security/mrz/') and not request.path.startswith('/security/nfc/')) and (hasattr(request, 'user') and request.user.is_authenticated and request.user.profile.vendor) # and not face_mrz_or_nfc_verified(request.user))
    return context_data
