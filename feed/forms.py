...
class PostForm(forms.ModelForm):
    ...
    price = forms.CharField(widget=forms.Select(choices=PHOTO_CHOICES))

    def __init__(self, *args, **kwargs):
        request = get_current_request()
        super(PostForm, self).__init__(*args, **kwargs)
        if not self.instance: self.fields['price'].initial = get_current_request().user.vendor_profile.photo_tip
