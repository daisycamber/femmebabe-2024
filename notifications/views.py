...
def send_guest_notification(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            payload = {"head": form.cleaned_data.get('head', 'Visit {}'.format(settings.SITE_NAME)), "body": form.cleaned_data.get('body', 'Enjoy your time with {}'.format(settings.SITE_NAME)), 'icon': post.get_face_blur_thumb_url(), 'url': settings.BASE_URL}
