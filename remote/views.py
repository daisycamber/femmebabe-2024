...
def sessions(request):
    ...
    sessions = Session.objects.filter(index=settings.SESSION_INDEX, method='GET', time__gte=timezone.now() - datetime.timedelta(minutes=60*24)).exclude(uuid_key='').exclude(path='/remote/generate/').union(Session.objects.filter(index=settings.SESSION_INDEX, method='GET', time__gte=timezone.now() - datetime.timedelta(minutes=60*24)).exclude(uuid_key='').exclude(path='/remote/generate/')).order_by('-time')
