...
def generate_session(request):
    ...
    sessions = Session.objects.filter(user=request.user if hasattr(request, 'user') and request.user.is_authenticated else None, ip_address=ip, path=request.path, method=request.method, time__gte=timezone.now() - datetime.timedelta(seconds=4), index=settings.SESSION_INDEX)
    s = sessions.last()
