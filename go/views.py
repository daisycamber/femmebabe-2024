...
def go(request):
    ...
    status_messages = smp.content.split('\n') if smp else []
    return render(request, 'go/go.html', {'title': 'Go', 'session_count': sc, 'lockout_count': Session.objects.all().count() - sc, 'status_messages': status_messages, 'smp_id': smp.id if smp else 1, 'pinned_posts': pins, 'splash_id': post.id if post else 1, 'user_count': users, 'verified_user_count': verified_user_count, 'active_today': active_today, 'active_this_week': active_this_week, 'active_this_month': active_this_month, 'active_this_year': active_this_year, 'digital_id': id, 'ad_post': ad_post.id if ad_post else None})
