...
#    active_this_year = User.objects.filter(is_active=True, profile__last_seen__gte=timezone.now()-datetime.timedelta(days=365)).count()
    ...
    return render(request, 'go/go.html', {'title': 'Go', 'session_count': sc, 'lockout_count': Session.objects.all().count() - sc, 'status_messages': status_messages, 'smp_id': smp.id if smp else 1, 'pinned_posts': pins, 'splash_id': post.id if post else 1, 'digital_id': id, 'ad_post': ad_post.id if ad_post else None})
