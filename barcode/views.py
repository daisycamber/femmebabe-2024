...
def scan_barcode(request):
    ...
    if not foreign and not request.user.faces.count():
    ...
    if (not back and (request.user.profile.id_front_scanned and not request.user.profile.id_back_scanned and not foreign and not request.user.profile.idscan_active and not request.GET.get('download'))):
        return redirect(request.path + get_qs(request.GET) + '&back=true')
    if ((request.user.profile.id_front_scanned and request.user.profile.id_back_scanned and not foreign and not request.user.profile.idscan_active and not request.GET.get('download'))):
        return redirect(request.GET.get('next') or '/')
    if request.method == 'POST' and not fraud_detect(request, True) and (request.user.profile.idscan_used < request.user.profile.idscan_plan if request.user.profile.idscan_active else True):
        request.user.profile.idscan_used = request.user.profile.idscan_used + 1
        request.user.profile.save()
        if request.user.profile.can_scan_id > timezone.now():
...
#                request.user.profile.can_scan_id = timezone.now() + datetime.timedelta(minutes=settings.MINUTES_PER_IDSCAN_STAFF)
        if is_valid:
            ...
            if back:
                ...
                scan.verified = True
                scan.save()
                if not foreign and not request.user.profile.idscan_active:
