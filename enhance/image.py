...
def enhance_post(post_id):
    ...
    shutil.copy(p.image_original.path, p.image.path)
    print('rotate')
...
def routine_enhance_post():
    ...
    for post in posts:
        if post.image and post.image_original and os.path.exists(post.image.path) and os.path.exists(post.image_original.path):
