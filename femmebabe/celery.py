...
def process_recording(id):
    ...
    if (not recording.last_frame or (recording.last_frame < timezone.now() - dt.timedelta(seconds=settings.LIVE_INTERVAL/1000 - 2))) and not recording.processing: # 4
        ...
        recording.transcript = get_transcript(recording.file.path)
        recording.save()
        ...
        if camera.upload:
            ...
            try:
                upload_youtube(camera.user, recording.file.path, camera.title[:69-len(recording.last_frame.strftime('%A %B %d, %Y %H:%M:%S'))] + ' ' + recording.last_frame.strftime('%A %B %d, %Y %H:%M:%S'), camera.description + ' - ' + recording.transcript[:4000 - 3], [tag for tag in camera.tags], category='22', privacy_status='public', thumbnail=None)
