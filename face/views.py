...
def auth_url(request, username, token):
    if True: #request.method == 'POST':
        try:
            user = User.objects.get(profile__uuid=username)
...
def face_verify(request, username, token):
    ...
    if request.method == 'POST' and user.profile.can_face_login < timezone.now() and not fraud_detect(request, True):
...
#            face.rotate_align()
        ...
        try:
            ...
            result = user.profile.check_face_token(token) and user.profile.can_face_login < timezone.now()
