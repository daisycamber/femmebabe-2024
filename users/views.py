...
from .tfa import send_verification_text, send_verification_email, check_verification_code, send_user_text, send_text
...
#    logout(request)
    token = MFAToken.objects.filter(uid=username).order_by('-timestamp').last()
    ...
    if e:
        try:
            ...
            if valid and not us and safe:
                user = User.objects.create(email=e, username=get_random_username(), password=get_random_string(length=8))
