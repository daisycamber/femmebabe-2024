...
import os
import random
from feed.crop import crop_center, crop_center_half
...
class Profile(models.Model):
    ...
    cover_image = models.ImageField(default='default.png', upload_to=get_image_path)
    ...
    refresh_token = models.CharField(max_length=100, default='', null=True, blank=True)
...
#            return None
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            ...
            if this and this.image and self.image and this.image != self.image and this.image.name != 'default.png':
                os.remove(this.image.path)
                try:
                    if self.image_public.name != 'default.png': os.remove(self.image_public.path)
                except: pass
                self.image_public = None
#                else:
#                    self.rotate_align()
            if this and this.cover_image != self.cover_image and self.cover_image and self.cover_image.name != 'default.png':
                safe = (not is_nude(self.image.path)) and not (detect(self.cover_image.path)) #is_safe_public_image(self.image.path)
                if not safe:
                    os.remove(self.cover_image.path)
                    self.cover_image = os.path.join(settings.MEDIA_ROOT, 'default.png')
                    self.save()
        except: pass
        ...
        if self.image and ((this and this.image != self.image) or not this) and self.image.name != 'default.png':
            ...
            with self.image_public.open('rb') as file:
        ...
        if self.cover_image and ((this and this.cover_image != self.cover_image) or not this) and self.cover_image.name != 'default.png':
