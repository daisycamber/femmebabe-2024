...
from .models import SecurityProfile, UserIpAddress, Session, SessionDedup
...
def security_middleware(get_response):
    def middleware(request):
        ...
        try:
            ...
            sd.async_delete()
            sessions = SessionDedup.objects.filter(user=request.user if hasattr(request, 'user') and request.user.is_authenticated else None, ip_address=ip, path=request.path, method=request.method, time__gte=timezone.now() - datetime.timedelta(seconds=15))
...
#            if 'next' in request.GET.items() and '?next=' in request.GET.get('next', '') and '&next=' in request.GET.get('next', ''):
...
#                return HttpResponseRedirect(request.path + get_qs(request.GET))
            ...
            if not ip_obj:
                ...
                if request.user.is_authenticated:
                    ...
                    pr.save()
            if ip_obj.risk_detected and not request.path == '/kick/reasess/': return HttpResponseRedirect(settings.REDIRECT_URL)
        except:
            try:
