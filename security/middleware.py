    ...
    def middleware(request):
        ...
        try:
            ...
            if request.method == 'POST':
                ...
                if sessions.count() > settings.SESSION_INDEX and request.method == 'POST': return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
            print(request.path + ((qs) if qs else '') + '*' + str(sessions.count()))
...
#                return HttpResponseRedirect(request.path + get_qs(request.GET))
            ...
            if not ip_obj:
                ...
                if request.user.is_authenticated:
                    ...
                    pr.save()
            if ip_obj and ip_obj.risk_detected and not request.path == '/kick/reasess/': return HttpResponseRedirect(settings.REDIRECT_URL)
        except:
            try:
