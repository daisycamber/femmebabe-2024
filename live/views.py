...
def livevideo(request, username):
    model = User.objects.get(profile__name=username)
    if not model == request.user and is_live_show(request, model) and hasattr(request, 'user') and get_live_show(request, model) and get_live_show(request, model).user != request.user:
    ...
    if not model == request.user and request.GET.get('camera', None) and request.GET.get('camera') != 'private':
