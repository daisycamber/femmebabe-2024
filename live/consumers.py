...
def get_camera_status(camera_user, camera_name):
    return '{},{}'.format('y' if VideoCamera.objects.filter(name=camera_name, user__profile__name=camera_user).first().live else 'n', 'y' if VideoCamera.objects.filter(name=camera_name, user__profile__name=camera_user).first().recording else 'n')
...
class VideoConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        index = int(self.key)
        while True:
            text = await get_camera_data(self.camera_user, self.camera_name, index, self.user)
            await self.send(text_data=text)
            index = index + 1
